...
//char Address0[5] = { 0xe7, 0xd3, 0xf0, 0x35, 0x77};
//char Address1[5] = { 0xc2, 0xc2, 0xc2, 0xc2, 0xc2};
	
//char Address_P3[5] = { 0xc9, 0xd3, 0xf0, 0x35, 0x77};	
//char Address_P2[5] = { 0xc8, 0xd3, 0xf0, 0x35, 0x77};
...
char NRF24L01_R_Get_CD(void) {
	return (NRF24L01_R_ReadReg(CD) & 1);
}
...
void NRF24L01_R_Set_Power(char Mode) {
	...
	Result = NRF24L01_R_ReadReg(CONFIG) & 0b01111101; // Read Conf. Reg. AND Clear bit 1 (PWR_UP) and 7 (Reserved)
	NRF24L01_R_WriteReg(W_REGISTER | CONFIG, Result | Mode);
}
...
void NRF24L01_R_Set_CH(char CH) {
	NRF24L01_R_WriteReg(W_REGISTER | RF_CH, (CH & 0b01111111)); // Clear bit 8
}
...
void NRF24L01_R_Set_ShockBurst(char Mode) {
	...
	NRF24L01_R_WriteReg(W_REGISTER | SETUP_RETR, Mode);
	
	NRF24L01_R_WriteReg(W_REGISTER | EN_AA, Mode);
}
...
void NRF24L01_R_Set_Address_Width(char Width) {
	NRF24L01_R_WriteReg(W_REGISTER | SETUP_AW, 0x03/*(Width & 3) -2*/);
}
...
void NRF24L01_R_Set_Device_Mode(char Device_Mode) {
	...
	Result = NRF24L01_R_ReadReg(CONFIG) & 0b01111110; // Read Conf. Reg. AND Clear bit 0 (PRIM_RX) and 7 (Reserved)
	NRF24L01_R_WriteReg(W_REGISTER | CONFIG, Result | Device_Mode);
}
...
void NRF24L01_R_Set_RX_Pipe(char PipeNum, char *Address, int AddressSize, char PayloadSize) {
	...
	Result = NRF24L01_R_ReadReg(EN_RXADDR);
...
void NRF24L01_R_Disable_All_Pipes(void) {
	NRF24L01_R_WriteReg(W_REGISTER | EN_RXADDR, 0);
}
...
char NRF24L01_R_Clear_Interrupts(void) {
	return NRF24L01_R_WriteReg(W_REGISTER | STATUSe, _RX_DR | _TX_DS | _MAX_RT);
}
...
void NRF24L01_R_Set_TX_Address(char *Address, int Size) {
	NRF24L01_R_WriteRegBuf(W_REGISTER | TX_ADDR, Address, Size);
}
...
char *Address, char Address_Width, char Size_Payload, char Tx_Power) {
	...
	//NRF24L01_R_CE_OUT; // Set Port DIR out
...
void NRF24L01_R_Write_TX_Buf(char *Buf, int Size) {
	NRF24L01_R_WriteRegBuf(W_REGISTER | W_TX_PAYLOAD, Buf, Size);
}
...
void NRF24L01_R_Read_RX_Buf(char *Buf, int Size) {
	NRF24L01_R_ReadRegBuf(R_RX_PAYLOAD, Buf, Size);
}
...
{	
/* Send pattern. */
	spi_put(&NRF24L01_R_SPI,TX_Data);

	/* Wait for transmission complete. */
	while(!(spi_is_tx_ok(&NRF24L01_R_SPI)));
	/* Read received data. */
	uint8_t result = spi_get(&NRF24L01_R_SPI);
